#OPTION('obfuscateOutput', TRUE);//JoinCrime
IMPORT $;
...
Join4:=JOIN(join3, Population, LEFT.State=RIGHT.State AND LEFT.Year=RIGHT.Year, trans(LEFT,RIGHT));

//Population added to the factors-usage of %safety.=pop-vics/pops*100
myRecCrime5:=RECORD
STRING30 State;
UNSIGNED Year;
UNSIGNED No_Of_Homicides;
...
/*
myRec6:=RECORD
STRING30 State:=Join5.State;
UNSIGNED Year:=Join5.Year;
UNSIGNED No_Of_Homicides:=Join5.No_Of_Homicides;
UNSIGNED No_Of_Living_With_Parents:=Join5.No_Of_Living_With_Parents;
UNSIGNED No_Of_JuvenileCrimes:=Join5.No_Of_JuvenileCrimes;
UNSIGNED No_Of_Property_Stolen:=Join5.No_Of_Property_Stolen;
UNSIGNED No_Of_Property_Recovered:=Join5.No_Of_Property_Recovered;
UNSIGNED No_Of_RapeCases:=Join5.No_Of_RapeCases;
UNSIGNED Population:=Join5.Population;
DECIMAL ViolentCrimePRC:=Join5.ViolentCrimePRC;
DECIMAL PropertyRecoveredRatio:=Join5.PropertyRecoveredRatio;
UNSIGNED ViolentScore:=0;
END;
*/
